...
### Key points
  ...
  * The pipeline consists of multiple stages (e.g., build, test, deploy) that work in sequence or parallel on different iterations of the software. A CICD pipeline can handle multiple versions or branches of the software simultaneously, allowing teams to work on different features or bug fixes in parallel. The pipeline consists of multiple stages that can work simultaneously on different aspects of the current software version or update.
